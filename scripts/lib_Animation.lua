...
function pickAndPlace(Crane, ContainerPick, Container, ContainerMover, 
    ...
    if boolReverse then
        ...
        WMove(ContainerMover, y_axis, transportHeight, speed*3)
        WTurn(Crane, y_axis, math.rad(placeRot), speed)
        WMove(ContainerMover, y_axis, placeHeight, speed*3 )
        Hide(Container)
        reset(Container)
        Show(ContainerPlace)
    else
        ...
        WMove(ContainerMover, y_axis, transportHeight, speed*3 )
        WTurn(Crane, y_axis, math.rad(placeRot), speed)
        WMove(ContainerMover, y_axis, pickHeight, speed*3)
        Hide(ContainerPick)
        Show(Container)
        WTurn(Crane, y_axis, math.rad(pickRot), speed)
        Show(ContainerPick)
...
function pack(...) return {...}, select("#", ...) end
...
function WaitForTurns(...)
    ...
    if not arg then
        echo("No arguments for WaitForTurns");
        return
    end
...
function WaitForMoves(...)
    ...
    if not arg then
        echo("No arguments for WaitForMoves");
        return
    end
...
function movePieceToPieceWorld(unitID, name, pieceheigth, target, speed)
    ...
    MovePieceToPos(name, -1 * (bx - wx), -1 * (by - wy) - pieceheigth,
                   -1 * (bz - wz), 0)

end

-- >Turn a piece towards a random direction
function turnPieceRandDir(piecename, speed, LIMUPX, LIMLOWX, LIMUPY, LIMLOWY,
                          LIMUPZ, LIMLOWZ)
    if not LIMUPX then
...
function movePieceRandDir(piecename, speed, LIMUPX, LIMLOWX, LIMUPY, LIMLOWY,
                          LIMUPZ, LIMLOWZ)
    if not LIMUPX then
        return
    else
...
function AmphibMoveThread(unitid, PivotPoints, pieces, updateCycle, moveRatio,
                          nlswimAnimation, nlstopSwimAnimation,
                          nloutOfWaterAnimation, nlbackIntoWaterAnimation,
                          nlwalkAnimation, nlstopWalkAnimation)
...
function idleLoop(Body, axis, FrontLeg, RearLeg, degree, BodyBackDeg, speed,
    ...
    for i = 1, #FrontLeg, 1 do
        ...
        if boolNoDown == false then WaitForTurns(FrontLeg[i].Down) end
    end

    for i = 1, #RearLeg, 1 do
        WaitForTurns(RearLeg[i].Up)
        if boolNoDown == false then WaitForTurns(RearLeg[i].Down) end
    end
...
function equiTurnIn(p1, p2, axis, degv, times)
    ...
    turnInTime(p1, axis, degv, times, math.deg(x_rad), math.deg(y_rad),
               math.deg(z_rad))
    x_rad, y_rad, z_rad = UnitScript.GetPieceRotation(p2)
    turnInTime(p2, axis, degv * -1, times, math.deg(x_rad), math.deg(y_rad),
               math.deg(z_rad))
...
function breath(p1, p2, axis, degv, speed, itteration, speedreduce,
    ...
    if finalfunction then finalfunction(p1, p2) end
end
...
function tP(piecename, x_val, y_val, z_val, speed, boolWaitForIT)
    if piecename == nil then
        echo("libAnimation::tP got nil piece " .. x_val, y_val)
    end
...
function syncTurn(unitID, piecename, x_val, y_val, z_val, speed)
    ...
    if maxv == 0 then
        ...
        turnSyncInSpeed(piecename, x_val, y_val, z_val, speed, x_deg, y_deg,
                        z_deg)
        return
    end
...
function tInTime(piecename, taxis, goalDeg, timeInMs, rot)
    ...
    turnInTime(piecename, taxis, goalDeg, timeInMs, math.deg(rot.x),
               math.deg(rot.y), math.deg(rot.z))
end

-- >Turns a piece in the speed necessary to arrive after x Milliseconds 
-- > overrirdes the spring shortes path turns
function turnInTime(piecename, taxis, goalDeg, timeInMs, x_startdeg, y_startdeg,
                    z_startdeg, boolWait)
    assert(z_startdeg)

    -- Gets the absolute Biggest Rotation
    startDeg = math.ceil(selectAxisValue(taxis, x_startdeg, y_startdeg,
                                         z_startdeg))
    ...
    if absoluteDeg <= 180 then
        ...
        if boolWait and boolWait == true then
            WaitForTurn(piecename, taxis)
        end
...
function tSyncIn(piecename, x_val, y_val, z_val, timeMS, lUnitScript)
    ...
    syncTurnInTime(piecename, x_val, y_val, z_val, timeMS, math.deg(x_rad),
                   math.deg(y_rad), math.deg(z_rad))
end

-- >Turns a piece on every axis in times 
function syncTurnInTime(piecename, x_goaldeg, y_goaldeg, z_goaldeg, timeMS,
                        x_curdeg, y_curdeg, z_curdeg)
    ...
    if x_goaldeg ~= x_curdeg then
        turnInTime(piecename, 1, (x_goaldeg), timeMS, x_curdeg, y_curdeg,
                   z_curdeg, false)
    end
    if y_goaldeg ~= y_curdeg then
        turnInTime(piecename, 2, (y_goaldeg), timeMS, x_curdeg, y_curdeg,
                   z_curdeg, false)
    end
    if z_goaldeg ~= z_curdeg then
        turnInTime(piecename, 3, (z_goaldeg), timeMS, x_curdeg, y_curdeg,
                   z_curdeg, false)
    end
...
function spinT(Table, axis, rdeg, speed, degup)
    if type(Table) == "number" then
        val = math.random(rdeg, rdeg + degup);
        Spin(Table, axis, math.rad(val), speed)
    end

    if not degup then
        for k, v in pairs(Table) do
            if v then Spin(v, axis, math.rad(rdeg), speed) end
        end
    else
        for k, v in pairs(Table) do
            if v then
...
function stopSpinT(Table, axis, speed)
    ...
    for i = 1, #Table do if Table[i] then StopSpin(Table[i], axis, speed) end end
end
...
function getElementsNStepsDown(Steps, Value)
    ...
    if Steps > 0 and type(Value) == "table" then
        ...
        for i = 1, #Value do
            ...
            if foundIt then table.insert(Tables, foundIt) end
        end
        return Tables
    end
end
...
function buildSkeleton(rootpiecename, keyPieceNrtable)
    ...
    for key, piecenumber in pairs(keyPieceNrtable) do
        ...
        if parent and parent == rootpiecename then SubPieces[key] = {} end
    end
...
function AlignPieceToPiece(unitID, pieceToAlign, PieceToAlignTo, speed,
                           boolWaitForIt, boolDebug, GlowPoints)
...
function moveExpPiece(piece, axis, targetPos, startPos, increaseval, startspeed,
                      endspeed, speedUpSlowDown)
    speed = startspeed
...
function relDist(pos, target)
    ...
    -- pos - target +
    if pos < 0 and target > 0 then rDist = math.abs(pos) + target end

    -- pos - target +
    if pos > 0 and target < 0 then rDist = (math.abs(target) + pos) * -1 end
...
function movePieceToPieceNoReset(unitID, piecename, pieceDest, speed, offset)
    ...
    diffx, diffy, diffz = relDist(orgx, gox), relDist(orgy, goy),
                          relDist(orgz, goz)
    -- echo("Diff:", -1*diffx,  diffy, diffz)

    syncMove(piecename, -1 * diffx + offset.x, diffy + offset.y,
             diffz + offset.z, speed)

    WaitForMoves(piecename)
end

-- >Moves a UnitPiece to a UnitPiece at speed
function movePieceToPiece(unitID, piecename, pieceDest, speed, offset,
                          forceUpdate, boolWaitForIT)
...
-- >Moves a UnitPiece to a UnitPiece at speed
function movePieceFromPieceToPiece(unitID, piecename, pieceDest, pieceStart,
                                   speed, offset, forceUpdate)
    reset(piecename, 0) -- last changeset
...
function moveUnitPieceToGroundPos(unitID, piecename, X, Z, speed, offset)
    if not piecename then
        error("No piecename given by " ..
                  UnitDefNames[Spring.GetUnitDefID(unitID)].name);
        return
    end
...
function stableConditon(legNr, q)
    return
        GG.MovementOS_Table ~= nil and GG.MovementOS_Table[unitID].stability >
            0.5 and
            GG.MovementOS_Table[unitID].quadrantMap[math.max(math.min(4, q), 1)] >
            0 or
            GG.MovementOS_Table[unitID].quadrantMap[math.max(
                math.min(4, legNr % 2), 1)] and
            GG.MovementOS_Table[unitID].quadrantMap[math.max(
                math.min(4, legNr % 2), 1)] > 0
end

-- Controlls One Feet- Relies on a Central Thread running and regular updates of each feet on his status
function feetThread(quadrant, degOffSet, turnDeg, nr, FirstAxisPoint, KneeT,
                    SensorPoint, Weight, Force, LiftFunction, LegMax,
                    WiggleFunc, ScriptEnviroment, SensorT)
    LMax = LegMax or 5
    oldHeading = 0
    Sleep(500)

    stabilize(quadrant, degOffSet, turnDeg, nr, FirstAxisPoint, KneeT,
              SensorPoint, Weight, Force, LiftFunction, ScriptEnviroment,
              SensorT)
    ...
    while true do
        while GG.MovementOS_Table[unitID].boolmoving == true do
            ...
            liftFeedForward(quadrant, degOffSet, turnDeg, nr, FirstAxisPoint,
                            KneeT, SensorPoint, Weight, Force, LiftFunction,
                            ScriptEnviroment)
            Sleep(100)
            stabilize(quadrant, degOffSet, turnDeg, nr, FirstAxisPoint, KneeT,
                      SensorPoint, Weight, Force, LiftFunction, SensorT)
            Sleep(100)
            pushBody(quadrant, degOffSet, turnDeg, nr, FirstAxisPoint, KneeT,
                     SensorPoint, Weight, Force, nr, ScriptEnviroment)

            Sleep(100)
            stabilize(quadrant, degOffSet, turnDeg, nr, FirstAxisPoint, KneeT,
                      SensorPoint, Weight, Force, LiftFunction,
                      ScriptEnviroment, SensorT)
            Sleep(100)
        end
...
function liftFeedForward(quadrant, degOffSet, turnDeg, nr, FirstAxisPoint,
    ...
    for i = 1, #KneeT, 1 do Turn(KneeT[i], x_axis, math.rad(-2), speed) end
    WaitForTurn(FirstAxisPoint, y_axis)
end
...
function stabilize(quadrant, degOffSet, turnDeg, nr, FirstAxisPoint, KneeT,
                   SensorPoint, Weight, Force, LiftFunction, ScriptEnviroment,
                   SensorT)
    ...
    for i = #KneeT, 1, -1 do
        ...
        if y - GroundHeight > 20 then -- Go down		
            ...
            tDeg = clamp(tDeg + 0.25 + propagatedCounterChange, -75,
                         math.max((1 / i) * 75, 25))
            ...
            tDeg = clamp(tDeg - 1.15 + propagatedCounterChange, -75,
                         math.max((1 / i) * 75, 25))
            tDeg = convertToNeg(tDeg)
...
function paintPatternPieces(ListOfPieces, ListOfCoords, sx, sy, sz)
    ...
    MovePieceToPos(ListOfPieces[1], ListOfCoords[1].x, ListOfCoords[1].y,
                   ListOfCoords[i].z)
    TurnPieceTowards(ListOfPieces[1], sx, sy, sz, 0)
    prevx, prevy, prevz = ListOfCoords[1].x, ListOfCoords[1].y,
                          ListOfCoords[i].z

    for i = 2, #ListOfCoords - 1 do
        MovePieceToPos(ListOfPieces[i], ListOfCoords[i].x, ListOfCoords[i].y,
                       ListOfCoords[i].z)
...
function moveUnitPieceToRelativeWorldPos(unitID, piecename, relX, relZ, speed,
                                         loffset)
    offset = loffset or 0
    x, globalHeightUnit, z = Spring.GetUnitPosition(unitID)
    x, z = relX - x, relZ - z
...
function moveSpeedCurve(piecename, axis, NumberOfArgs, now, timeTotal, distToGo,
                        Offset, ...)
    local arg = arg;
    if (not arg) then
        arg = {...};
...
function stuckInPlaceAvoidance(unitID, times, intervall)
    ...
    while true do
        ...
        if intervallCounter > intervall then
            ...
            Spring.AddUnitImpulse(unitID, dx * impulsfactor, dy * impulsfactor,
                                  dz * impulsfactor)
        end
        Sleep(times)
    end
...
function DropPieceToGround(unitID, piecename, speed, boolWait, boolHide,
                           ExplodeFunction, SFXCOMBO)
...
function generateSknakeOnAPlaneDefaults(cPceDescLst)
    for iNumerated, arm in pairs(PceDescLst) do
        ...
        if not arm.Piece then
            echo("libAnimation::snakeOnAPlane - No Valid Piece in Arm");
            return
        end

        armTcX, _, armTcZ, armTdX, _, armTdZ =
            Spring.GetUnitPiecePosDir(unitID, arm.Piece)
        ...
        if PceDescLst[iNumerated + 1] and PceDescLst[iNumerated + 1].Piece then
            successorPiece = PceDescLst[iNumerated + 1].Piece
        end
        ...
        if not arm.lx then
            arm.lx, arm.lz = absDistance(armTcX, sucTcX),
                             absDistance(armTcX, sucTcZ)
        end
...
function snakeOnAPlane(unitID, cPceDescLst, FirstSensor, WindowDescriptorList,
                       axis, speed, tolerance, boolPartStepExecution, boolWait)
    local PceDescLst = cPceDescLst -- {Piece_Pos_Deg_Length_PointIndex_boolGateCrossed_}List

    -- early error out
    if not PceDescLst then
        echo("libAnimation::snakeOnAPlane - No Valid PieceCegTable");
        return
    end

    if WindowDescriptorList == nil then
        echo("libAnimation::snakeOnAPlane - No Valid Goals to move");
        return
    end

    -- Working Defaults
    -- if not defined ArmCenter - define Arm as centered in UnitSpace
    if not PceDescLst.ArmCenterOffset then
        PceDescLst.ArmCenterOffset = {ox = 0, oz = 0}
    end
    ...
    boolResolved = false
    if not WindowDescriptorList then
        echo("snakeOnAPlane:: Not WindowDescriptorList delivered");
        return
    end
    ...
    vOrg = {};
    vOrg.x, vOrg.y, vOrg.z = WindowDescriptorList[1].vx, 0,
                             WindowDescriptorList[1].vz
    echoT(PceDescLst)

    for i = 1, #WindowDescriptorList do end
    -- func
    ...
    while boolResolved == false do
        ...
        while boolAlgoRun == false do
            ...
            for Index = #PceDescLst, 1, -1 do
                ...
                x, y, z, dx, dy, dz = Spring.GetUnitPiecePosDir(unitID,
                                                                PceDescLst[Index]
                                                                    .Piece)

                local PieceStartPoint = makeVector(x, y, z)
                px, py, pz, pdx, pdy, pdz =
                    Spring.GetUnitPiecePosDir(unitID, PceDescLst[math.min(
                                                  Index + 1, #PceDescLst)].Piece)
                local PieceEndPoint = makeVector(px, py, pz)

                ppx, ppy, ppz, ppdx, ppdy, ppdz =
                    Spring.GetUnitPiecePosDir(unitID, PceDescLst[math.min(
                                                  Index + 1, #PceDescLst)].Piece)
                local PrevGatePoint = makeVector(ppx, ppy, ppz)

                -- CheckCenterPastPoint_PointIndex 
                boolPastCenterPoint = checkCenterPastPoint(
                                          midVector(PieceStartPoint,
                                                    PieceEndPoint),
                ...
                if boolPastCenterPoint == true and
                    PceDescLst[Index].boolGateCrossed == false then
                    ...
                    --
                    PceDescLst[Index].boolGateCrossed =
                        checkCenterPastPoint(
                            midVector(PieceStartPoint, PieceEndPoint),
                            WindowDescriptorList[nextGoal], PrevGatePoint)

                    if PceDescLst[Index].boolGateCrossed == true then
                        PceDescLst[Index].PointIndex =
                            PceDescLst[Index].PointIndex + 1
                    end
                ...
                elseif boolPastCenterPoint == true and
                    PceDescLst[Index].boolGateCrossed == true then
...
function isSnakeAtMax(PceDescLst, SnakePoints)
    ...
    for i = 1, #PceDescLst do
        px, py, pz, dx, dy, dz = Spring.GetUnitPiecePosDir(unitID,
                                                           PceDescLst[i].Piece)
        pgx, pgy, pgz, dgx, dgy, dgz = Spring.GetUnitPiecePosDir(unitID,
                                                                 SnakePoints[PceDescLst[Index]
                                                                     .PointIndex])
...
function mulVectorS4Mat(mat, vec)
    ...
    for u = 0, 3, 1 do
        sum = 0
        for v = 1, 4, 1 do sum = sum + (enVec[v] * mat[u * 4 + v]) end
...
function TurnPieceList(ScriptEnviroment, PieceList, boolTurnInOrder,
                       boolWaitForTurn, boolSync)

    for i = 1, table.getn(PieceList), 5 do

        if boolSync == false then
            tP(PieceList[i], PieceList[i + 1], PieceList[i + 2],
               PieceList[i + 3], PieceList[i + 4], boolTurnInOrder)
        else
            if not PieceList[i] then
                echo("TurnPieceList piece " .. i .. " missing")
            else
                x_deg, y_deg, z_deg = ScriptEnviroment.GetPieceRotation(
                                          PieceList[i])

                turnSyncInSpeed(PieceList[i], PieceList[i + 1],
                                PieceList[i + 2], PieceList[i + 3],
                                PieceList[i + 4], math.deg(x_deg),
                                math.deg(y_deg), math.deg(z_deg))
            end
        end
    end

    if boolWaitForTurn == true and boolTurnInOrder == false then
        for i = 1, table.getn(PieceList), 5 do WaitForTurns(PieceList[i]) end
    end
end
...
function moveT(t, axis, dist, speed, boolInstantUpdate, start, endindex)
    start = start or 1
    if (not type(t) == "table") then echo("Not a table"); return end
...
function unfoldAnimation(ListOfPieces, specialeffectsfunction, unitID, maxSpeed)
    ...
    for k, v in pairs(ListOfPieces) do
        SizeSortedTable = binaryInsertTable(SizeSortedTable,
                                            PieceIDSizeTable[v], v, k)
        HeightSortedTable = binaryInsertTable(HeightSortedTable,
                                              PieceIDHeightTable[v], v, k)
    end

    ClosedTable = {}
    AllreadyVisiblePieces[SizeSortedTable[#SizeSortedTable].key] =
        PieceIDSizeTable[SizeSortedTable[#SizeSortedTable].key]
    ...
    for i = 1, #HeightSortedTable, 1 do
        if HeightSortedTable[i].value ~= StartPiece then
            ...
            movePieceToPiece(unitID, HeightSortedTable[i].value,
                             PieceBiggerThenMe, 0)
            ...
            AllreadyVisiblePieces[HeightSortedTable[i].value] =
                PieceIDSizeTable[HeightSortedTable[i].value]
            -- ShowTheBiggest
        end
    end
...
function dropPieceTillStop(unitID, piece, speedPerSecond, VspeedMax, lbounceNr,
                           boolSpinWhileYouDrop, bounceConstant, driftFunc)
    if not unitID or not piece or not speedPerSecond or not VspeedMax then
        return
    end
    x, globalHeightUnit, z = Spring.GetUnitPosition(unitID)
    ...
    while bump < bounceNr do
        ...
        vec.y = vec.y +
                    clampMaxSign(vec.vy * force % (speed * factorT) - 1 * speed,
                                 factorT * speedMax)
        vec.x = vec.x +
                    clampMaxSign(vec.vx * force % (speed * factorT),
                                 factorT * speedMax)
        vec.z = vec.z +
                    clampMaxSign(vec.vz * force % (speed * factorT),
                                 factorT * speedMax)

        mP(piece, vec.x, vec.y, vec.z, factorT * speed)

        -- shrink vec with sqrt as a approximation for air resistance
        vec.vx = clampMaxSign(math.sqrt((math.abs(vec.vx) ^ 1.414)) *
                                  Signum(vec.vx), 1)
        vec.vz = clampMaxSign(math.sqrt((math.abs(vec.vz) ^ 1.414)) *
                                  Signum(vec.vz), 1)
        ...
        if gh - y > 5 then
            ...
            vec.vx, vec.vy, vec.vz = (clampMaxSign(dx * 0.5, 1)),
                                     vec.vy * -0.75, (clampMaxSign(dz * 0.5, 1))
...
function turnT(t, axis, degs, speed, boolInstantUpdate, boolWait)
    if boolInstantUpdate then
        for i = 1, #t, 1 do Turn(t[i], axis, math.rad(degs), 0, true) end
        return
    end

    if not speed or speed == 0 then
        for i = 1, #t, 1 do Turn(t[i], axis, math.rad(degs), 0) end
...
function wiggle(piecename, xval, yval, zval, timetotal, timeMin, timeMax,
                overshootfactor)

    timeSnippet = math.random(timeMin, timeMax)
    x, y, z = math.random(0, xval) * randSign(),
              math.random(0, yval) * randSign(),
              math.random(0, zval) * randSign()
    ...
    sigN = 1
    for i = 1, 4 do
        tSyncIn(piecename, x + x * copy, y + y * copy, z + z * copy,
                math.ceil(math.abs(timeSnippet)))
...
function adaptiveAnimation(configTable, inPeace, id, ScriptEnviroment)
    ...
    for i = 1, infoT.nr do
        ...
        StartThread(feetThread, math.floor(
                        math.min(math.max(0, (i * turnOffset) / 360), 1) * 4),
                    (-190 + (85) * i), maxDeg, i, infoT.feetTable.firstAxis[i],
                    infoT.feetTable.Knees[i], infoT.sensorTable[i],
                    infoT.ElementWeight or 10, infoT.FeetLiftForce or 2,
                    infoT.LiftFunction, infoT.Height, infoT.WiggleFunc,
                    ScriptEnviroment, infoT.tipTable[i])
    end
    ...
    while true do
        ...
        local one, three = GG.MovementOS_Table[unitID].quadrantMap[1],
                           GG.MovementOS_Table[unitID].quadrantMap[3]
        local two, four = GG.MovementOS_Table[unitID].quadrantMap[2],
                          GG.MovementOS_Table[unitID].quadrantMap[4]
        ...
        if BoolStable == false then
            GG.MovementOS_Table[unitID].stability =
                math.min(1, (1 / total) * GG.MovementOS_Table[unitID].stability)
        else
            GG.MovementOS_Table[unitID].stability = 1
        end

        Move(infoT.centerNode, y_axis,
             GG.MovementOS_Table[unitID].stability * Height, 3)
        ...
        for i = 1, infoT.nr, 1 do
            ...
            if (RelHeading < 0) then
                RelHeading = (RelHeading + 360) * -1
            end

            if boolTurning == true then
                speed = math.random(5, 15) / 100
                Turn(infoT.feetTable.firstAxis[i], y_axis,
                     math.rad(degOffSet + clamp(RelHeading, -25, 25) * -1),
                     speed)
                turnT(infoT.feetTable.Knees[i], y_axis,
                      clamp(RelHeading, -10, 10) * -1, speed, false, true)
            else
...
function TurnPieceTowardsUnit(piecename, unitToTurnToo, Speed, overrideVec)
    ...
    TurnPieceTowardsPoint(piecename, x, y, z, Speed, overrideVec.x,
                          overrideVec.y, overrideVec.z)
end
-- > movePiece to Vector
...
function TurnPieceTowardsPoint(piecename, x, y, z, Speed, lox, loy, loz, limVec)
    ...
    px, py, pz, pvec.x, pvec.y, pvec.z =
        Spring.GetUnitPiecePosDir(unitID, piecename)
    ...
    tPrad(piecename, limVec.x * (math.atan2(vec.y, vec.z) + ox),
          ...
          limVec.z * (math.atan2(vec.x, vec.y) + oz), Speed)
end
...
function resetT(tableName, speed, boolShowAll, boolWait, boolIstantUpdate,
    ...
    if interValStart and interValEnd then
        for i = interValStart, interValEnd do
            ...
            if boolShowAll and tableName[i] then Show(tableName[i]) end
        end
    else
        for k, v in pairs(tableName) do
            reset(k, lspeed, false, boolIstantUpdate or true)
            if boolShowAll and v then Show(v) end
        end
    end

    if lboolWait == true then WaitForTurns(tableName) end
end

-- > applys a physics function to a detached  Piece from a Unit @EventStreamFunction
function unitRipAPieceOut(unitID, rootPiece, shotVector, factor, parabelLength,
                          boolSurvivorHeCanTakeIt)
    ...
    if env and env.script.Explode and env.script.Hide then
        ...
        Spring.UnitScript.CallAsUnit(unitID, env.script.Explode, rootPiece,
                                     env.SFX.FALL + env.SFX.NO_HEATCLOUD)
...
function rippleHide(array, startIndex, endIndex, Sleeptime)
    ...
    for i = startIndex, endIndex do
        Hide(array[i])
        Sleep(Sleeptime)
        if array[i - 1] then Show(array[i - 1]) end
    end
end
function movePieceInParabel(unitID, pieceName, yDegreeOffset, xDegreeOffset,
                            valueToGo, valueStart, offsetY, steepness, env,
                            speed)
...
function recResetT(Table, speed)
    if type(Table) == "table" then
        for k, v in pairs(Table) do recResetT(v, speed) end
...
function reset(piecename, speed, boolWaitForIT, boolIstantUpdate)
    ...
    if type(piecename) ~= "number" then
        Spring.Echo("libAnimation::reset:Invalid piecename-got " .. piecename ..
                        " of type " .. type(piecename) .. " instead");
        assert(true == false);
    end
...
function objectFalling(objectname, weight, step, OVect, term)
    ...
    sizeX, sizeY, sizeZ = Spring.GetUnitPieceCollisionVolumeData(unitID,
                                                                 objectname)
    ...
    while true do
        ...
        if OPosY - size < Spring.GetGroundHeight(oPosX, oPosZ) then
            ...
            OVec.x, OVec.y, Ovec.z = groundX + OVec.x,
                                     (OVec.y * -1) * weight + groundY,
                                     OVec.z + groundZ
        end
...
function waveATable(Table, axis, lfoonction, lsignum, lspeed, lfuncscale,
                    ltotalscale, boolContra, offset)
    ...
    for i = 1, nr do
        ...
        if type(Table[i]) == "table" then
            waveATable(Table[i], axis, func, signum, speed, funcscale,
                       totalscale, boolContra, offset)
        else
...
function followPath(unitID, pieceName, pathTable, speed, delay, boolWaitForMove,
    ...
    for i = 1, #pathTable do
        ...
        movePieceFromPieceToPiece(unitID, pieceName, pieceNum,
                                  pathTable[math.max(i - 1, 1)], speed, offset)

        if boolWaitForMove == true then WaitForMoves(pieceName) end
        Sleep(delay)
    end
end

function equiTurnAboveGround(turnPiece, counterPiece, SensorPiece, targetY,
                             eqStart, eqMin, eqMax, speed, resolution, axis)
...
function hoverSegway(SystemPiece, PivotPiece, PowerPiece, HoverPiece,
                     Resolution, rotOffsetPivotPiece, rotOffsetPowerPiece, axis,
                     rotationValueFunction, activeFunction, speed, restoreSpeed)
    ...
    Diff = {
        x = PivotPos.x - PowerPos.x,
        y = PivotPos.y - PowerPos.y,
        z = PivotPos.z - PowerPos.z
    }
    ...
    if Diff.y > 1 then
        ...
        Turn(PowerPiece, axis,
             -rotOffsetPivotPiece - rotOffsetPowerPiece - math.random(5, 12),
             speed / 3)
...
function stillMoving(personNr, dramatisPersona3d)
    ...
    if (true ==
        Spring.UnitScript.IsInMove(dramatisPersona3d[personNr][2], z_axis) or
        true ==
        Spring.UnitScript.IsInTurn(dramatisPersona3d[personNr][1], y_axis)) then
        return true
...
function carSenderJobFunc(dramatisPersona3d, personNr)
    ...
    targetHeight = 120
    luckOne = 0
    if math.random(0, 1) == 1 then luckOne = math.random(12, 50) / 10 end
    tempDirAction = 1
    ...
    if dramatisPersona3d[personNr][6] == 1 then
        ...
        Move(dramatisPersona3d[personNr][2], z_axis, targetDist,
             (tempSpeed * 6) + luckOne)
        tempDirAction = 8
    ...
    elseif dramatisPersona3d[personNr][6] == 2 then
        ...
        Turn(dramatisPersona3d[personNr][1], y_axis, math.rad(targetdegree),
             ((dramatisPersona3d[personNr][10]) / 100) + 0.1) -- 0.3
        tempDirAction = 1
    ...
    elseif dramatisPersona3d[personNr][6] == 4 then
        ...
        Move(dramatisPersona3d[personNr][2], z_axis, NtargetDist,
             (tempSpeed * 6) + luckOne)
        tempDirAction = 2
    elseif dramatisPersona3d[personNr][6] == 8 then
        ...
        Move(dramatisPersona3d[personNr][2], y_axis, targetHeight,
             tempSpeed * 1.4)
        dramatisPersona3d[personNr][13] = targetHeight
        Turn(dramatisPersona3d[personNr][1], y_axis, math.rad(Ntargetdegree),
             ((dramatisPersona3d[personNr][10]) / 100) + 0.1) -- 0.3
        tempDirAction = 4
    end
...
function startPieceOS(pieceName, Signal, setOfLoopTasks)
	...
	for i=1, #setOfLoopTasks do
		...
		StartThread(cyclicLoopFunctions[task.func], pieceID, Signal, task.arg)	
	end
end

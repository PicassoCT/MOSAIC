		...
		if buildID  then
			if boolLaunchReady == false then
				...
				if buildDefID == launcherStepDefID  then
					...
					Move(RocketAttach,upaxis,math.min( -rocketHeigth + stepIndex*stepHeight, 0), 10)
					Move(Elevator,upaxis,math.min( -rocketHeigth + stepIndex*stepHeight, 0), 10)
					showT(TablesOfPiecesGroups["Step"],0,stepIndex)
					Hide(TablesOfPiecesGroups["WIP"][ stepIndex ])
					Show(TablesOfPiecesGroups["WIP"][math.min(#TablesOfPiecesGroups["WIP"], stepIndex + 1)])
					showT(TablesOfPiecesGroups["Gantry"],0,math.min(#TablesOfPiecesGroups["Gantry"] ,stepIndex *2))
...
function workCycle()
	while boolWorkCycle == true do
		...
		if boolBuilding == true then
			for i=1,4 do
				StartThread(moveToWorkBasePos,i)
			end
		end
		...
		while startFrame ~= endFrame do
			startFrame = Spring.GetGameFrame()
				for i=1,4 do
						robotID = i
						WaitForTurns(
							TablesOfPiecesGroups["AAxis"][robotID],		
						 	TablesOfPiecesGroups["BAxis"][robotID],		
						 	TablesOfPiecesGroups["CAxis"][robotID],		
						 	TablesOfPiecesGroups["DAxis"][robotID],		
						 	TablesOfPiecesGroups["EAxis"][robotID])

...
function testAnimation()
    while true do
        PlayAnimation("SIDEWALK", nil, 2.0)
        Sleep(100)
    end
end
function Landing()
...
function setupAnimation()
    ...
    local offsets = constructSkeleton(unitID, map.aimrot, {0, 0, 0});
...
function PlayAnimation(animname, piecesToFilterOutTable, speed)
    ...
    for i = 1, #anim do
        ...
        for j = 1, #commands do
            ...
            moveFactor = 1.0
            randoffset = 0.0
            if cmd.c == "move" then moveFactor = 5.0 end

            if cmd.r then randoffset = math.random(-cmd.r, cmd.r) end

            if piecesToFilterOutTable[cmd.p] == nil and cmd.p then
                animCmd[cmd.c](cmd.p, cmd.a,
                               axisSign[cmd.a] * (cmd.t + randoffset),
                               cmd.s * speedFactor * moveFactor)

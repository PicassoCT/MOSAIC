...
function allTheWayDown()
    while true do
        ...
        moveUnitPieceToGroundPose(unitID, center, 0, 0, math.abs(h) + 1,
                                  -h - Tieferlegen)
        Sleep(250)
    end
...
function recursiveTurn(element, timeInMs)
    if element.pId then
        tSyncIn(element.pId, element.xr, element.yr, element.zr, timeInMs,
                Spring.UnitScript)
    else
        for k, v in pairs(element) do
            if type(v) == "table" then recursiveTurn(v, timeInMs) end
        end
    end
end
...
function recursiveHideShow(element, boolShow)
    if element.pId then
        ...
        for k, v in pairs(element) do
            if type(v) == "table" then recursiveHideShow(v) end
        end
    end
end

function limitLowLeg(val) return math.max(-175, math.min(0, val)) end
...
function genericSexPos()
    ...
    local grindPos = 0;
    if math.random(1, 4) > 3 then grindPos = 180 end
    local sideLines = 0;
    if math.random(1, 4) > 3 then sideLines = 90 * randSign() end
    ...
    local sidesign = -1
    local grindSign = 1;
    if grindDir == 180 then grindSign = -1 end
    ...
    if sideLines ~= 0 then
        herLegAngleOrg = math.random(0, 20);
        herLegInwardRot = math.random(-20, 20)
    end
    if herLegAngleOrg > 70 then herLegInwardRot = math.random(-30, 30) end

    local orgPos = {
        she = {
            head = {pId = p2_head, xr = math.random(-25, -10), yr = 0, zr = 0},
            up_body = {
                pId = p2_u_body,
                xr = math.random(-15, -5),
                yr = 0,
        ...
        he = {
            ...
            head = {pId = p1_head, xr = math.random(-15, -5), yr = 0, zr = 0},
            up_body = {
                pId = p1_u_body,
                xr = math.random(-10, 10),
                yr = 0,
            ...
            leg = {
                left = {
                    up = {pId = p1_left_u_leg, xr = hisLeg * -1, yr = 0, zr = 0},
                    low = {
                        pId = p1_left_l_leg,
                ...
                right = {
                    up = {pId = p1_right_u_leg, xr = 0, yr = 0, zr = 0},
                    low = {pId = p1_right_l_leg, xr = 0, yr = 0, zr = 0}
                }
            },
            low_body = {
                pId = p1_l_body,
    ...
    poundPos.she.low_body.xr = poundPos.she.low_body.xr + math.random(0, 5) *
                                   randSign()
    ...
    for i = 1, iterrations do
        ...
        poundPos.she.leg.right.low.xr = limitLowLeg(
                                            orgPos.she.leg.right.low.xr +
                                                (2 * legOffset) * randSign())

        legOffset = math.random(5, 20)
        poundPos.she.leg.left.up.xr = orgPos.she.leg.left.up.xr - legOffset
        poundPos.she.leg.left.low.xr = limitLowLeg(
                                           orgPos.she.leg.left.low.xr +
                                               (2 * legOffset) * randSign())
        ...
        armOffset = math.random(-20, 20)
        poundPos.he.arm.right.up.zr = orgPos.he.arm.right.up.zr +
                                          (armOffset * sidesign * 3)
        poundPos.he.arm.right.low.zr = orgPos.he.arm.right.low.zr - armOffset *
                                           sidesign
...
function FuckFest()
    ...
    while true do genericSexPos() end
end

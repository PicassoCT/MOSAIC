...
function getMapCultureMap(mapName)
    ...
    if mapToCultureDictionary[mapName] then
        return mapToCultureDictionary[mapName]
    end
end

function getInstanceCultureOrDefaultToo(defaultCulture)
    if GG.InstanceCulture then
...
function getManualCivilianBuildingMaps(mapName)
    ...
    if ManualCivilianBuildingPlacement[mapName] then
        return ManualCivilianBuildingPlacement[mapName]
    end
end
...
function isNearCityCenter(x, z, GameConfig)
    if not GG.innerCityCenter then
        return false
    end
...
function getDeterministicRotationOffsetForDistrict(districtID, cultureDeviation, xDiv1000, zDiv1000)
    if not GG.DistrictRotationDeterministic then
        GG.DistrictRotationDeterministic = {}
    end
    x, z = math.ceil(xDiv1000 / 10), math.ceil(zDiv1000 / 10)
    if not GG.DistrictRotationDeterministic[x] then
        GG.DistrictRotationDeterministic[x] = {}
    end
    if not GG.DistrictRotationDeterministic[x][z] then
        GG.DistrictRotationDeterministic[x][z] = {}
    end
    if not GG.DistrictRotationDeterministic[x][z][districtID] then
        GG.DistrictRotationDeterministic[x][z][districtID] = math.random(0, 9) * 45
    end
...
function getCultureDependantRandomOffsets(culture, loc)
    ...
    if culture == Cultures.arabic then
        rotDegOffset =
            getDeterministicRotationOffsetForDistrict(
            getLocationHash(loc.x, loc.z),
            5,
            math.ceil(loc.x / 1000),
            math.ceil(loc.z / 1000)
        )
        return {
            xRandOffset = 20,
            zRandOffset = 20,
...
function getBaseTypeName(name)
    if name:match("house") then
        return "house"
    end
    if name:match("civilian") then
        return "civilian"
    end
    if name:match("truck") then
        return "truck"
    end
end
...
function setAerosolCivilianBehaviour(unitID, TypeOfBehaviour)
    ...
    if env and env.startAerosolBehaviour then
        Spring.UnitScript.CallAsUnit(unitID, env.startAerosolBehaviour, TypeOfBehaviour)
        return true
    end
    return false
end
...
function setAssemblyProducedUnitsToTeam(assemblyID, teamToTurnOverID)
    ...
    if env and env.TurnProducedUnitsOverToTeam then
        Spring.UnitScript.CallAsUnit(assemblyID, env.TurnProducedUnitsOverToTeam, teamToTurnOverID)
        return true
    end
    return false
end
...
function getUnitScaleTable(UnitDefNames)
    ...
    for name, v in pairs(UnitDefNames) do
        factor = 0.3
        if realScaleTable[name] then
            factor = realScaleTable[name]
        end
...
function getUnitSide(unitID)
    ...
    if ProtagonUnitTypeList[defID] then
        return "protagon"
    end
    if AntagonUnitTypeList[defID] then
        return "antagon"
    end
    return "gaia"
end
...
function isPrayerTime()
    ...
    return GG.GameConfig.instance.culture == Cultures.arabic and equal(percent, 0.25, 0.025) or
        equal(percent, 0.75, 0.025)
end
...
function isRushHour()
    ...
    if hours > 6 and hours < 9 or hours > 12 and hours < 14 or hours > 16 and hours < 20 then
        return true
    end
end
...
function attachDoubleAgentToUnit(traitorID, teamToTurnTo, boolRecursive)
    attachingTo =
        traitorID ..
        " a " ..
            UnitDefs[Spring.GetUnitDefID(traitorID)].name ..
                " of team " .. Spring.GetUnitTeam(traitorID) .. " is a double agent for team " .. teamToTurnTo
    echo(attachingTo)
    if not GG.DoubleAgents then
        GG.DoubleAgents = {}
    end
    ...
    hoverAboveFunc = function(persPack)
        ...
        if GG.DoubleAgents[persPack.traitorID] and GG.DoubleAgents[persPack.traitorID] ~= persPack.iconID then
            return boolEndFunction, nil
        end

        if persPack.boolDoneFor then
        ...
        if doesUnitExistAlive(persPack.iconID) == false then
            persPack.iconID =
                createUnitAtUnit(
                persPack.teamToTurnTo,
                "doubleagent",
                persPack.traitorID,
                x - 1,
                y + persPack.heightAbove,
                z
            )
        ...
        if persPack.boolRecursive and persPack.boolRecursive == true then
            if not persPack.ListOfBuildUnits then
                persPack.ListOfBuildUnits = {}
            end
        ...
        if
            persPack.startFrame + 1 < Spring.GetGameFrame() and persPack.boolCloakedAtLeastOnce == true and
                boolUnitIsCloaked == false
         then
...
function createRewardEvent(teamid, returnOfInvestmentM, returnOfInvestmentE)
    ...
    rewarderProcess = function(evtID, frame, persPack, startFrame)
        Spring.AddTeamResource(persPack.teamId, "metal", persPack.returnOfInvestmentM / persPack.rewardCycles)
        Spring.AddTeamResource(persPack.teamId, "energy", persPack.returnOfInvestmentE / persPack.rewardCycles)
    ...
    GG.EventStream:CreateEvent(rewarderProcess, persPack, Spring.GetGameFrame() + 1)
end
...
function registerRevealedUnitLocation(unitID)
    ...
    if not GG.RevealedLocations then
...
function giveParachutToUnit(id, x, y, z, boolDelayed)
    if not GG.ParachutPassengers then
        GG.ParachutPassengers = {}
    end
    ...
    if Spring.GetGameFrame() < 1 or boolDelayed then
        ...
        GG.EventStream:CreateEvent(delayedParachutSpawn, persPack, Spring.GetGameFrame() + 1)
    else
...
function delayedKillProjectile(id, timeInMS)
    ...
    GG.EventStream:CreateEvent(delayedKill, persPack, Spring.GetGameFrame() + 1)
end
...
function removeUnit(teamID, unit)
    ...
    if parent then
        GG.InheritanceTable[teamID][parent][unit] = nil
    end
...
function computateClusterNodes(housePosTable, GameConfig)
    timeFactor =
        math.abs(math.sin(math.pi * Spring.GetGameFrame() / GameConfig.civilianGatheringBehaviourIntervalFrames)) -- [0 - 1]
    ...
    InfluenceStateMachines = {
        ...
        [AerosolTypes.wanderlost] = function(lastState, currentState, unitID)
            ...
            if currentState == InfStates.Outbreak then
                ...
                f =
                    (Spring.GetGameFrame() % GG.GameConfig.Aerosols.wanderlost.VictimLiftime) /
        ...
        [AerosolTypes.tollwutox] = function(lastState, currentState, unitID)
            ...
            if gf % 30 == 0 and maRa() then
                for i = 1, 3 do
                    ...
                    turnT(Spring.GetUnitPieceMap(unitID), i, math.rad(val), 30.125)
                end
            end
            ...
            if ed and (not ad or currentState == InfStates.Standalone) then
                ...
                if enemyDistance and enemyDistance < 20 then
                    process(
                        getAllNearUnit(unitID, attackDistance),
                        function(id)
...
function getAllTeamsOfType(teamType, UnitDefs)
    ...
    process(
        ...
        function(tid)
            ...
            if tid ~= gaiaTeamID and false == isDead and (string.find(side, teamType)) then
                returnT[tid] = tid
            end
        end
    )
...
function getOperativeName(hash, sex, culture)
names={
male ={"Jason","James", "Jack" },
female = {"Jill"}
}
surname= {
    male ={},
...
function getRandomCultureNames(culture)
    names = {
        arabic = {
            ...
            family = {
                ...
                "Akram Hamoodi ",
                " Akram Hamoodi ",
                "El Abideen Akram Hammodi ",
                "Akram Hamoody Hamoodi ",
                "Iyad Hamoodi ",
                "Muhammad Hamoodi",
                "Elhuda Saad Hamoodi ",
                "Abed Hamoodi ",
    ...
    return names[culture].sur[math.random(1, #names[culture].sur)] ..
        " " .. names[culture].family[math.random(1, #names[culture].family)]
end

function getIdleTokken()
    if not GG.CurrentIdleNr then
        GG.CurrentIdleNr = 0
    end

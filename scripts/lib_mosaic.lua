...
ListOfMediaAdvertisementFileLength = 
{
...
function buildRunDeterministicAdvertisement()
    ...
    for i=1, # identifierList do
        ...
        for f=1, #allFilesInPath do
            fileName = allFilesInPath[f]
            element = {
                path = fileName, 
                time = ListOfMediaAdvertisementFileLength[fileName]
                }
    ...
    for i=1, # identifierList do
        ...
        if identifier == "5Binding" and 
            getDeterministicRandom(hash, math.abs(hash - GG.DeterministicCounterAdvertsiement)) > math.ceil(hash * 0.25) then

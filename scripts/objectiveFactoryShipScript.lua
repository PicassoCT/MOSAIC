...
function pickPlaceJob(SliderPart,CranePart, Container, range)
	...
	while true do
		if maRa()==true then -- PlaceJob
			...
			Show(Container)
			Length_Location= math.floor(math.random(0,range))*container_dim_length
			WMove(SliderPart, length_axis, Length_Location, speed)
			WMove(CranePart,side_axis, math.random(-7,7)*container_dim_width, speed)

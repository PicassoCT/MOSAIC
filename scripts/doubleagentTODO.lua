...
function createDoubleAgent(id, teamToTurnTo)
	
gameConfig = getGameConfig()

doubleAgentID = createUnitAtUnit(teamToTurnTo, "doubleagent", id, 0, gameConfig.doubleAgentHeigth , 0)
...
hoverAboveFunc = function( persPack)
					...
					-- if unit is activated then turn agent and self-destroy
					--TODO


					return boolDoneFor, persPack
				end

createStreamEvent(doubleAgentID, hoverAboveFunc, 1, {
													myTeam = teamToTurnTo, 
													toTrackID = id, 
													heightAbove = gameConfig.doubleAgentHeigth}
													)
end

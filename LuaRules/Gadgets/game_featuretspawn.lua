...
local function PushKillFeature(self, ...)
    self[#self + 1] = { ... }
end

local function PushCreateFeature(self, ...)
    self[#self + 1] = { ... }
end

if GG.FeaturesToSpawn == nil then GG.FeaturesToSpawn = { PushCreateFeature = PushCreateFeature } end
if GG.FeaturesToKill == nil then GG.FeaturesToKill = { PushKillFeature = PushKillFeature } end
        ...
        if GG.FeaturesToSpawn and GG.FeaturesToSpawn[1] then
            ...
            for i = 1, #cur, 1 do
                --	Spring.Echo(unpack(cur[i]))
						...
						assert(cur[i][5], "teamID missing in PushCreateFeature "..cur[i][1])

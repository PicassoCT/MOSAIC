  ...
  if (effects) then
    for i=1,#effects do
      ...
      if (fx.options.heightFactor) then
		...
		--allows for dynamic effects based on unitid
		if type(fx.options.size)=="string" then
		f=loadstring("function () id="..unitID..fx.options.size)
			if type(f)== "function" then fx.options.size=f  else fx.options.size= 128 end
		
		elseif (fx.options.radiusFactor) then
    ...
    for i=1,#allPlayers do
      ...
      if (playerName == "Picasso") then
        ...
        if (errorLog~="") then
          local cmds = {
            "say ------------------------------------------------------",
            "say Hello jK i have been meddled and paddled by Picasso! This is an outrage:",
          }

...
if (not gadgetHandler:IsSyncedCode()) then -- unsynced gadget
    ...
    local function GetBrdfTexture() return brdfLut:GetTexture() end

    local function GetEnvTexture() return envLut:GetTexture() end

    function gadget:DrawGenesis()
        if brdfLut then brdfLut:Execute(false) end
    ...
    function gadget:DrawWorldPreUnit() -- after IBL textures are rendered into, but before units are drawn
        if envLut then
            ...
            if envLutDebug then envLutDebug = false end
        end
    end
    ...
    function gadget:Initialize()
        ...
        local brdfLutClass = VFS.Include(
                                 "luarules/gadgets/include/GenBrdfLut.lua")
        ...
        local envLutClass =
            VFS.Include("luarules/gadgets/include/GenEnvLut.lua")
        if envLutClass then

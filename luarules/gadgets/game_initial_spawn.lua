...
if gadgetHandler:IsSyncedCode() then
    ...
    -- Vars
    ----------------------------------------------------------------
    local antagonStartUnitDefID = UnitDefNames.operativepropagator.id
    local protagonStartUnitDefID = UnitDefNames.operativeinvestigator.id

    local validStartUnits = {
        [antagonStartUnitDefID] = true,
        [protagonStartUnitDefID] = true
    }
    ...
    function GetFFAStartPoints()
        ...
        if not ffaStartPoints and
            VFS.FileExists("luarules/configs/ffa_startpoints.lua") then
    ...
    function isPlayerNewbie(pID)
        local name, _, isSpec, tID, _, _, _, _, pRank =
            Spring.GetPlayerInfo(pID, false)
        ...
        if enabled then
            ...
            for i = 1, #teamList do
                ...
                if teamID ~= gaiaTeamID then
                    ...
                    local _, _, _, _, teamSide, teamAllyID =
                        spGetTeamInfo(teamID, false)
                    if teamSide == 'protagon' then
                        spSetTeamRulesParam(teamID, startUnitParamName,
                                            protagonStartUnitDefID)
                    else
                        spSetTeamRulesParam(teamID, startUnitParamName,
                                            antagonStartUnitDefID)
                    end
                    ...
                    spSetTeamRulesParam(teamID, 'isNewbie', newbieParam,
                                        {public = true}) -- visible to all; some widgets (faction choose, initial queue) need to know if its a newbie -> they unload

                    -- record that this allyteam will spawn something
                    local _, _, _, _, _, allyTeamID =
            ...
            if useFFAStartPoints then GetFFAStartPoints() end
    ...
    function gadget:RecvLuaMsg(msg, playerID)
        ...
        if startUnit and validStartUnits[startUnit] then
            local _, _, playerIsSpec, playerTeam =
            ...
            if not playerIsSpec then
                ...
                spSetTeamRulesParam(playerTeam, startUnitParamName, startUnit,
                                    {allied = true, public = false}) -- visible to allies only, set visible to all on GameStart
                return true
            end
        end
    end
    ...
    function gadget:AllowStartPosition(playerID, teamID, readyState, x, y, z)
        ...
        if xmin >= xmax or zmin >= zmax then
            ...
            local isOutsideStartbox = (xmin + 1 >= x) or (x >= xmax - 1) or
                                          (zmin + 1 >= z) or (z >= zmax - 1) -- the engine rounds startpoints to integers but does not round the startbox (wtf)
            if isOutsideStartbox then return false end
        end
        ...
        for otherTeamID, startpoint in pairs(startPointTable) do
            ...
            local sameAllyTeam = (allyTeamID ==
                                     select(6, Spring.GetTeamInfo(otherTeamID,
                                                                  false)))
            if (sx > 0) and tooClose and sameAllyTeam and not sameTeam then
                Spring.SendMessageToPlayer(playerID,
                                           "You cannot place your start position too close to another player")
                return false
            end
        end
    ...
    function gadget:GameStart()
        ...
        if Game.startPosType == 1 and ffaStartPoints and
            ffaStartPoints[nSpawnTeams] and #(ffaStartPoints[nSpawnTeams]) ==
            nSpawnTeams then
    ...
    function SetPermutedSpawns(nSpawns, idsToSpawn)
        ...
        local perm = {}
        for i = 1, nSpawns do perm[i] = i end
    ...
    function SpawnTeamStartUnit(teamID, allyTeamID)
        ...
        if Game.startPosType == 2 then
            if ((not startPointTable[teamID]) or
                (startPointTable[teamID][1] < 0)) then
    ...
    function SpawnStartUnit(teamID, x, z)
        ...
        if Spring.GetTeamRulesParam(teamID, 'isNewbie') == 1 then
            if math.random() > 0.5 then
                startUnit = protagonStartUnitDefID
            else
                startUnit = antagonStartUnitDefID
            end
        end
        ...
        spSetTeamRulesParam(teamID, startUnitParamName, startUnit,
                            {public = true}) -- visible to all (and picked up by advpllist)
    ...
    function gadget:GameFrame()
        if enabled then gadgetHandler:RemoveGadget(self) end
    end
    ...
    local fontfile = "luaui/fonts/" ..
                         Spring.GetConfigString("bar_font2", "Exo2-SemiBold.otf")
    local vsx, vsy = Spring.GetViewGeometry()
    local fontfileScale = (0.5 + (vsx * vsy / 5700000))
    local fontfileSize = 50
    local fontfileOutlineSize = 10
    local fontfileOutlineStrength = 1.4
    local font = gl.LoadFont(fontfile, fontfileSize * fontfileScale,
    ...
    function gadget:ViewResize(viewSizeX, viewSizeY)
        ...
        if (fontfileScale ~= newFontfileScale) then
            ...
            gl.DeleteFont(font)
            font = gl.LoadFont(fontfile, fontfileSize * fontfileScale,
                               fontfileOutlineSize * fontfileScale,
                               fontfileOutlineStrength)
        end
    end
    ...
    function gadget:GameSetup(state, ready, playerStates)
        ...
        if gameStarting == nil and
            ((Spring.GetPlayerTraffic(SYSTEM_ID, NETMSG_STARTPLAYING) or 0) > 0) then -- ugly but effective (can also detect by parsing state string)
            gameStarting = true
        end

        -- if we can't choose startpositions, no need for ready button etc
        if Game.startPosType ~= 2 or ffaMode then return true, true end
    ...
    function correctMouseForScaling(x, y)
        ...
        x = x - (((x / vsx) - buttonScreenCenterPosX) * vsx) *
                ((uiScale - 1) / uiScale)
        y = y - (((y / vsy) - buttonScreenCenterPosY) * vsy) *
                ((uiScale - 1) / uiScale)
        return x, y
    end
    ...
    function gadget:MousePress(sx, sy)
        ...
        if sx > readyX - bgMargin and sx < readyX + readyW + bgMargin and sy >
            readyY - bgMargin and sy < readyY + readyH + bgMargin and
            Spring.GetGameFrame() <= 0 and Game.startPosType == 2 and
            gameStarting == nil and not spec then
    ...
    function gadget:MouseRelease(x, y) return false end
    ...
    function gadget:Initialize()
        ...
        readyButton = gl.CreateList(function()
            ...
            gl.Color(0, 0, 0, 0.75)
            RectRound(-((readyW / 2) + bgMargin), -((readyH / 2) + bgMargin),
                      ((readyW / 2) + bgMargin), ((readyH / 2) + bgMargin), 6)
            gl.Color(1, 1, 1, 0.11)
            RectRound(-readyW / 2, -readyH / 2, readyW / 2, readyH / 2, 4)
            gl.Color(1, 1, 1, 1)
        end)
        -- create ready button
        readyButtonHover = gl.CreateList(
                               function()
                -- draws background rectangle
                gl.Color(0.15, 0.12, 0, 0.75)
                RectRound(-((readyW / 2) + bgMargin),
                          -((readyH / 2) + bgMargin), ((readyW / 2) + bgMargin),
                          ((readyH / 2) + bgMargin), 6)
                RectRound(readyX - bgMargin, readyY - bgMargin,
                          readyX + readyW + bgMargin,
                          readyY + readyH + bgMargin, 5)
    ...
    function gadget:DrawScreen()
        ...
        if enabled then
            ...
            if not readied and readyButton and Game.startPosType == 2 and
                gameStarting == nil and not spec and not isReplay then
                -- if not readied and readyButton and not spec and not isReplay then

                if Script.LuaUI("GuishaderInsertRect") then
                    Script.LuaUI.GuishaderInsertRect(
                        readyX + (readyW / 2) -
                            (((readyW / 2) + bgMargin) * uiScale), readyY +
                            (readyH / 2) - (((readyH / 2) + bgMargin) * uiScale),
                        readyX + (readyW / 2) +
                            (((readyW / 2) + bgMargin) * uiScale), readyY +
                            (readyH / 2) + (((readyH / 2) + bgMargin) * uiScale),
                        'ready')
                ...
                if x > readyX - bgMargin and x < readyX + readyW + bgMargin and
                    y > readyY - bgMargin and y < readyY + readyH + bgMargin then
                ...
                font:Begin()
                font:Print(colorString .. "Ready", -((readyW / 2) - 12.5),
                           -((readyH / 2) - 9.5), 25, "o")
                font:End()
                gl.Color(1, 1, 1, 1)
            end
            ...
            if gameStarting and not isReplay then
                ...
                local text = colorString .. "Game starting in " ..
                                 math.max(1, 3 - math.floor(timer)) ..
                                 " seconds..."
                font:Begin()
                font:Print(text, vsx * 0.5 - font:GetTextWidth(text) / 2 * 17,
                           vsy * 0.75, 17, "o")
                font:End()
            end
            gl.PopMatrix()
            ...
            if unsupportedAI then
                font:Print(
                    "\255\200\200\200Unsupported AI  (" .. unsupportedAI ..
                        ")\n\nYou need \255\255\255\255DAI\255\200\200\200, \255\255\255\255KAIK \255\200\200\200or \255\255\255\255Chickens  \255\200\200\200(or NullAI)\n\n\255\130\130\130closing in... " ..
                        math.floor(timer3), vsx / 2, vsy / 2, vsx / 95, "con")
            else
                font:Print(
                    "\255\200\200\200Unsupported engine\n\nYou need at least version  \255\255\255\255" ..
                        minEngineVersionTitle ..
                        "\n\n\255\130\130\130closing in... " ..
                        math.floor(timer3), vsx / 2, vsy / 2, vsx / 95, "con")
            end
            font:End()
            if Script.LuaUI("GuishaderInsertRect") then
                Script.LuaUI.GuishaderInsertRect(0, 0, vsx, vsy,
                                                 'unsupportedquit')
            end
            if timer3 <= 0 then Spring.SendCommands("QuitForce") end
        end
    end

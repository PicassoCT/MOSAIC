...
if gadgetHandler:IsSyncedCode() then
    ...
    function gadget:RecvLuaMsg(inMsg, playerID)
        if inMsg:sub(1, 2) == validation and
            (inMsg:sub(3, HEADER_LENGHT + 2) == HEADER_SEL_UNCOMPRESSED or
                inMsg:sub(3, HEADER_LENGHT + 2) == HEADER_SEL_COMPRESSED) then
            SendToUnsynced("selectionUpdate", playerID, inMsg:sub(7),
                           inMsg:sub(6, 6) == "c")
            return true
        end
    end
    ...
    function gadget:Initialize()
        gadgetHandler:AddSyncAction("selectionUpdate",
                                    handleSelectionUpdateEvent)
    end
    ...
    function handleSelectionUpdateEvent(_, playerID, msg, compressed)
        ...
        if not spec or not fullView then
            local _, _, targetSpec, _, allyTeamID =
                GetPlayerInfo(playerID, false)
            if targetSpec or allyTeamID ~=
                select(5, GetPlayerInfo(myPlayerID, false)) then
                return
            end
        end
    ...
    function gadget:CommandsChanged(id, params, options) sendSelectedUnits() end
    ...
    function gadget:Update()
        ...
        if fullSelectionUpdateInt ~= 0 and timeSeconds % fullSelectionUpdateInt ==
            0 then
    ...
    function sendSelectedUnits()
        ...
        for i = 1, #units do
            ...
            if not myLastSelectedUnits[unitId] then
                ...
                if addCount > unitLimitPerFrame then break end
            end
        end

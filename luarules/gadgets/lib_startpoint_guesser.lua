...
function IsSteep(x, z)
    ...
    if math.abs(a1) > mtta or math.abs(a2) > mtta or math.abs(a3) > mtta or
        math.abs(a4) > mtta then
...
function GuessOne(teamID, allyID, xmin, zmin, xmax, zmax, startPointTable)
    ...
    if not GG.metalSpots then return -1, -1 end
    local metalSpots = GG.metalSpots
    if metalSpots == false then return -1, -1 end
    ...
    for i = 1, #metalSpots do
        ...
        local isWithinStartBox = (xmin < mx) and (mx < xmax) and (zmin < mz) and
                                     (mz < zmax)
        local isWithinWalkRadius = (mx >= xmin - walkRadius) and
                                       (mx <= xmax + walkRadius) and
                                       (mz >= zmin - walkRadius) and
                                       (mz <= zmax + walkRadius)
        ...
        for _, startpoint in pairs(startPointTable) do -- we avoid enemy startpoints too, to prevent unnecessary explosions and to deal with the case of having no startboxes
            ...
            local isWithinClaimRadius = ((sx - mx) * (sx - mx) + (sz - mz) *
                                            (sz - mz) <= (claimRadius) *
                                            (claimRadius))
...
function GuessTwo(teamID, allyID, xmin, zmin, xmax, zmax, startPointTable) -- TODO: make this more efficient, atm it akes 1-2 sec to run
    ...
    while true do -- i hate lua
        ...
        local isWithinStartBox = (xmin < 16 * x) and (16 * x < xmax) and
                                     (zmin < 16 * z) and (16 * z < zmax)
        if isWithinStartBox then
        ...
        for z = 1, mmapz, zres2 do
            ...
            for _, startpoint in pairs(startPointTable) do -- we avoid enemy startpoints too, to prevent unnecessary explosions and to deal with the case of having no startboxes
                ...
                local isWithinClaimRadius =
                    ((sx - x * 16) * (sx - x * 16) + (sz - z * 16) *
                        (sz - z * 16) <= (claimRadius2) * (claimRadius2))
            ...
            if isFree and m > 0.1 then
                ...
                for _, p in ipairs(points) do
                    if m and (p.x - x) * (p.x - x) + (p.z - z) * (p.z - z) <=
                        (claimRadius2 / 16) * (claimRadius2 / 16) and
                        math.abs(p.y - y) <= claimHeight then
                        p.m = p.m + m
                    end
                end
            end
    ...
    -- return best
    if best then return (points[best].x) * 16, (points[best].z) * 16 end

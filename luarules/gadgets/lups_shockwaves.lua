...
if (gadgetHandler:IsSyncedCode()) then
    ...
    for i = 1, #WeaponDefs do
        ...
        if (not customParams.lups_noshockwave) then
            local speed = 1
            local life = 1
            local normalShockwave = (wd.damageAreaOfEffect > 70 and
                                        not wd.paralyzer and
                                        not customParams.disarmdamageonly)
    ...
    function gadget:Explosion(weaponID, px, py, pz, ownerID)
        ...
        if shockwave then
            ...
            elseif shockwave.special == 1 then
                ...
                SendToUnsynced("lups_shockwave", px, py, pz, shockwave.growth,
                               shockwave.life)
            end
        end
        return false
    ...
    local function SpawnShockwave(_, px, py, pz, growth, life, strength,
        ...
        if (desintergrator) then
            ...
            Lups.AddParticles('SphereDistortion', {
                pos = {px, py, pz},
                life = life,
                strength = strength,
                growth = growth

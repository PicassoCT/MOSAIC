...
if (gadgetHandler:IsSyncedCode()) then
    local napalmWeapons = {}
    local wantedList = {} -- // find napalms    for i = 1, #WeaponDefs do
        local wd = WeaponDefs[i]
        if (wd.description:find("Napalm") or (wd.customParams and
            (wd.customParams.lups_napalm_fx or wd.customParams.lups_heat_fx))) then
            Script.SetWatchWeapon(wd.id, true)
            napalmWeapons[wd.id] = true
            wantedList[#wantedList + 1] = wd.id
        end
    end
    local napalmExplosions = {} -- // Speed-ups    local SendToUnsynced =
        SendToUnsynced
    function gadget:Explosion_GetWantedWeaponDef() return wantedList end
    ...
    local function SpawnNapalmFX(data)
        local nFXkey = WeaponDefs[data[1]].customParams and
                           WeaponDefs[data[1]].customParams.lups_napalm_fx or
                           "default"
        local hFXkey = WeaponDefs[data[1]].customParams and
                           WeaponDefs[data[1]].customParams.lups_heat_fx or
                           "default"
    ...
    local function GameFrame()
        ...
        for i = 1, napalmCount do SpawnNapalmFX(explosions[i]) end
    end

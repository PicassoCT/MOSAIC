-- NOTE this file is currently overriden by basecontent
...
local function mynext(...)
    ...
    if not whiteList[t] then
        Spring.Log(HANDLER_BASENAME, "error",
                   '*** A gadget is misusing pairs! Report this with full infolog.txt! ***')
        ...
        if isMission then
            Spring.Log(HANDLER_BASENAME, "error",
                       "Error depth: " .. DepthValue % DepthMod + 1,
                       DepthValue % DepthMod + 1)
        else
            error("Error depth: " .. DepthValue % DepthMod + 1,
                  DepthValue % DepthMod + 1) -- breaks mission_runner
        end
    end
    return i, v
end
...
local reverseCompatAllowStartPosition = not Spring.Utilities
                                            .IsCurrentVersionNewerThan(103, 629)
--------------------------------------------------------------------------------

function pgl() -- (print gadget list)  FIXME: move this into a gadget
    for k, v in ipairs(gadgetHandler.gadgets) do
        Spring.Echo(string.format("Gadget: %3i  %3i  %s", k, v.ghInfo.layer,
                                  v.ghInfo.name))
    end
end
...
local callInLists = {
    ...
    "UnitCreated", "UnitFinished", "UnitReverseBuilt", "UnitFromFactory",
    "UnitDestroyed", "RenderUnitDestroyed", "UnitExperience", "UnitIdle",
    "UnitCmdDone", "UnitPreDamaged", "UnitDamaged", "UnitStunned", "UnitTaken",
    "UnitGiven", "UnitEnteredRadar", "UnitEnteredLos", "UnitLeftRadar",
    "UnitLeftLos", "UnitSeismicPing", "UnitLoaded", "UnitUnloaded",
    "UnitCloaked", "UnitDecloaked", -- optional
    -- "UnitUnitCollision",
    -- "UnitFeatureCollision",
    -- "UnitMoveFailed",
    "StockpileChanged", -- Feature CallIns
    "FeatureCreated", "FeatureDestroyed", -- Projectile CallIns
    "ProjectileCreated", "ProjectileDestroyed", -- Shield CallIns
    "ShieldPreDamaged", -- Misc Synced CallIns
    "Explosion",
    ...
    "AllowWeaponTarget", "AllowWeaponInterceptTarget", -- unsynced
    "DrawUnit", "DrawFeature", "DrawShield", "DrawProjectile",
    "RecvSkirmishAIMessage", -- COB CallIn  (FIXME?)
    "CobCallback", -- Unsynced CallIns
    "Update", "DefaultCommand", "DrawGenesis", "DrawWorld", "DrawWorldPreUnit",
    "DrawWorldShadow", "DrawWorldReflection", "DrawWorldRefraction",
    "DrawScreenEffects", "DrawScreenPost", "DrawScreen", "DrawInMiniMap",
    "RecvFromSynced", -- moved from LuaUI
    "KeyPress", "KeyRelease", "MousePress", "MouseRelease", "MouseMove",
    "MouseWheel", "IsAbove", "GetTooltip",

    -- FIXME -- not implemented  (more of these?)
    "WorldTooltip", "MapDrawCmd", "GameSetup", "DefaultCommand", -- Save/Load
    "Save", "Load", -- FIXME: NOT IN BASE
    "UnitCommand", "UnitEnteredWater", "UnitEnteredAir", "UnitLeftWater",
    "UnitLeftAir", "UnsyncedHeightMapUpdate"
}
...
local function IsSyncedCode() return isSyncedCode end
...
local function ripairs(t) return rev_iter, t, (1 + #t) end
...
function gadgetHandler:Initialize()
    ...
    for k, gf in ipairs(gadgetFiles) do
        ...
        if (gadget) then table.insert(unsortedGadgets, gadget) end
    end
...
function gadgetHandler:LoadGadget(filename)
    ...
    if (chunk == nil) then
        Spring.Log(HANDLER_BASENAME, LOG.ERROR,
                   'Failed to load: ' .. basename .. '  (' .. err .. ')')
        return nil
    end
    ...
    if (knownInfo) then
        if (knownInfo.active) then
            Spring.Log(HANDLER_BASENAME, LOG.ERROR,
                       'Failed to load: ' .. basename .. '  (duplicate name)')
            return nil
        end
    else
...
function gadgetHandler:NewGadget()
    ...
    for k, v in pairs(System) do gadget[k] = v end
    ...
    gadget.include = function(f) return VFS.Include(f, gadget, VFSMODE) end
    ...
    gh.UpdateCallIn =
        function(_, name) self:UpdateGadgetCallIn(name, gadget) end
    gh.RemoveCallIn =
        function(_, name) self:RemoveGadgetCallIn(name, gadget) end

    gh.RegisterCMDID = function(_, id) self:RegisterCMDID(gadget, id) end
    ...
    gh.IsMouseOwner = function(_) return (self.mouseOwner == gadget) end
    gh.DisownMouse = function(_)
        if (self.mouseOwner == gadget) then self.mouseOwner = nil end
    end
...
local function ArrayInsert(t, f, g)
    if (f) then
        ...
        for i, v in ipairs(t) do
            ...
            if (layer >= v.ghInfo.layer) then index = i + 1 end
        end
...
function gadgetHandler:RemoveGadget(gadget)
    ...
    self.knownGadgets[name].active = false
    -- Spring.Echo(name)
    if (gadget.Shutdown) then gadget:Shutdown() end
    ...
    for id, g in pairs(self.CMDIDs) do
        if (g == gadget) then self.CMDIDs[id] = nil end
    end
...
function gadgetHandler:UpdateCallIn(name)
    ...
    if ((#self[listName] > 0) or (name == 'GotChatMsg') or
        (name == 'RecvFromSynced')) then
        local selffunc = self[name]
        _G[name] = function(...) return selffunc(self, ...) end
    else
        _G[name] = nil
    end
...
function gadgetHandler:UpdateGadgetCallIn(name, g)
    ...
    if (ciList) then
        ...
        Spring.Log(HANDLER_BASENAME, LOG.ERROR,
                   'UpdateGadgetCallIn: bad name: ' .. name)
    end
end
...
function gadgetHandler:RemoveGadgetCallIn(name, g)
    ...
    if (ciList) then
        ...
        Spring.Log(HANDLER_BASENAME, LOG.ERROR,
                   'RemoveGadgetCallIn: bad name: ' .. name)
    end
end

function gadgetHandler:UpdateCallIns()
    for _, name in ipairs(callInLists) do self:UpdateCallIn(name) end
end
...
function gadgetHandler:EnableGadget(name)
    ...
    if (not ki) then
        Spring.Log(HANDLER_BASENAME, LOG.ERROR,
                   "EnableGadget(), could not find gadget: " .. tostring(name))
        return false
    end
    if (not ki.active) then
        Spring.Echo('Loading:  ' .. ki.filename)
        local order = gadgetHandler.orderList[name]
        if (not order or (order <= 0)) then self.orderList[name] = 1 end
...
function gadgetHandler:DisableGadget(name)
    ...
    if (not ki) then
        Spring.Log(HANDLER_BASENAME, LOG.ERROR,
                   "DisableGadget(), could not find gadget: " .. tostring(name))
        return false
    end
    if (ki.active) then
...
local function FindGadgetIndex(t, w)
    for k, v in ipairs(t) do if (v == w) then return k end end
    return nil
end

local function FindLowestIndex(t, i, layer)
    for x = (i - 1), 1, -1 do
        if (t[x].ghInfo.layer < layer) then return x + 1 end
    end
    return 1
end
...
function gadgetHandler:FindGadget(name)
    ...
    for k, v in ipairs(self.gadgets) do
        if (name == v.ghInfo.name) then return v, k end
    end
    return nil
end
...
function gadgetHandler:RegisterGlobal(owner, name, value)
    if ((name == nil) or (_G[name]) or (self.globals[name]) or
...
function gadgetHandler:SetGlobal(owner, name, value)
    if ((name == nil) or (self.globals[name] ~= owner)) then return false end
    _G[name] = value
    return true
end
...
function gadgetHandler:GetHourTimer() return hourTimer end

function gadgetHandler:GetViewSizes() return self.xViewSize, self.yViewSize end

function gadgetHandler:RegisterCMDID(gadget, id)
    if not LOG_SECTION then LOG_SECTION = "ERROR" end
    if not id then
...
function gadgetHandler:GamePreload()
    for _, g in ipairs(self.GamePreloadList) do g:GamePreload() end
    return
end

function gadgetHandler:GameStart()
    for _, g in ipairs(self.GameStartList) do g:GameStart() end
    return
end
...
function gadgetHandler:GameFrame(frameNum)
    for _, g in ipairs(self.GameFrameList) do 
        --local name = g.ghInfo.name or "UNKNOWN NAME"
        --Spring.Echo("CallGameFrame:"..name)
...
function gadgetHandler:RecvFromSynced(...)
    if (actionHandler.RecvFromSynced(...)) then return end
    for _, g in ipairs(self.RecvFromSyncedList) do
        if (g:RecvFromSynced(...)) then return end
    end
    return
end
...
function gadgetHandler:RecvLuaMsg(msg, player)
    for _, g in ipairs(self.RecvLuaMsgList) do
        if (g:RecvLuaMsg(msg, player)) then return true end
    end
    return false
end
...
function gadgetHandler:ViewResize(vsx, vsy)
    for _, g in ipairs(self.ViewResizeList) do g:ViewResize(vsx, vsy) end
    return
end
...
function gadgetHandler:GameOver(winners)
    for _, g in ipairs(self.GameOverList) do g:GameOver(winners) end
    return
end

function gadgetHandler:GameID(gameID)
    for _, g in ipairs(self.GameIDList) do g:GameID(gameID) end
    return
end

function gadgetHandler:TeamDied(teamID)
    for _, g in ipairs(self.TeamDiedList) do g:TeamDied(teamID) end
    return
end

function gadgetHandler:PlayerAdded(playerID)
    for _, g in ipairs(self.PlayerAddedList) do g:PlayerAdded(playerID) end
    return
end

function gadgetHandler:PlayerChanged(playerID)
    for _, g in ipairs(self.PlayerChangedList) do g:PlayerChanged(playerID) end
    return
end
...
function gadgetHandler:DrawUnit(unitID, drawMode)
    for _, g in ipairs(self.DrawUnitList) do
        if (g:DrawUnit(unitID, drawMode)) then return true end
    end
    return false
end
...
function gadgetHandler:DrawFeature(featureID, drawMode)
    for _, g in ipairs(self.DrawFeatureList) do
        if (g:DrawFeature(featureID, drawMode)) then return true end
    end
    return false
end
...
function gadgetHandler:DrawShield(unitID, weaponID, drawMode)
    for _, g in ipairs(self.DrawShieldList) do
        if (g:DrawShield(unitID, weaponID, drawMode)) then return true end
    end
    return false
end
...
function gadgetHandler:DrawProjectile(projectileID, drawMode)
    for _, g in ipairs(self.DrawProjectileList) do
        if (g:DrawProjectile(projectileID, drawMode)) then return true end
    end
    return false
end
...
function gadgetHandler:RecvSkirmishAIMessage(aiTeam, dataStr)
    for _, g in ipairs(self.RecvSkirmishAIMessageList) do
        ...
        if (dataRet) then return dataRet end
    end
end

function gadgetHandler:CommandFallback(unitID, unitDefID, unitTeam, cmdID,
                                       cmdParams, cmdOptions, cmdTag)
    for _, g in ipairs(self.CommandFallbackList) do
        local used, remove = g:CommandFallback(unitID, unitDefID, unitTeam,
                                               cmdID, cmdParams, cmdOptions,
                                               cmdTag)
        if (used) then return remove end
    end
    return true -- remove the command
end

function gadgetHandler:AllowCommand(unitID, unitDefID, unitTeam, cmdID,
                                    cmdParams, cmdOptions, cmdTag, synced)
    for _, g in ipairs(self.AllowCommandList) do

        if (not g:AllowCommand(unitID, unitDefID, unitTeam, cmdID, cmdParams,
                               cmdOptions, cmdTag, synced)) then
            return false
        end
    end
    return true
...
function gadgetHandler:AllowStartPosition(playerID, teamID, readyState, cx, cy,
                                          cz, rx, ry, rz)
    if reverseCompatAllowStartPosition then
        cx, cy, cz, playerID, readyState, rx, ry, rz = playerID, teamID,
                                                       readyState, cx, cy, cz,
                                                       rx, ry
    end
    for _, g in ipairs(self.AllowStartPositionList) do
        if (not g:AllowStartPosition(playerID, teamID, readyState, cx, cy, cz,
                                     rx, ry, rz)) then return false end
    end
    return true
end

function gadgetHandler:AllowUnitCreation(unitDefID, builderID, builderTeam, x,
                                         y, z, facing)
    for _, g in ipairs(self.AllowUnitCreationList) do
        if (not g:AllowUnitCreation(unitDefID, builderID, builderTeam, x, y, z,
                                    facing)) then return false end
    end
    return true
end

function gadgetHandler:AllowUnitTransfer(unitID, unitDefID, oldTeam, newTeam,
                                         capture)
...
function gadgetHandler:AllowUnitBuildStep(builderID, builderTeam, unitID,
                                          unitDefID, part)
    for _, g in ipairs(self.AllowUnitBuildStepList) do
        if (not g:AllowUnitBuildStep(builderID, builderTeam, unitID, unitDefID,
                                     part)) then return false end
    end
    return true
end

function gadgetHandler:AllowFeatureBuildStep(builderID, builderTeam, featureID,
                                             featureDefID, part)
    for _, g in ipairs(self.AllowFeatureBuildStepList) do
        if (not g:AllowFeatureBuildStep(builderID, builderTeam, featureID,
                                        featureDefID, part)) then
            return false
        end
    end
    return true
...
function gadgetHandler:AllowWeaponTargetCheck(attackerID, attackerWeaponNum,
                                              attackerWeaponDefID)
    for _, g in ipairs(self.AllowWeaponTargetCheckList) do
        if (not g:AllowWeaponTargetCheck(attackerID, attackerWeaponNum,
                                         attackerWeaponDefID)) then
            return false
        end
    end
    return true
...
function gadgetHandler:AllowWeaponTarget(attackerID, targetID,
    ...
    for _, g in ipairs(self.AllowWeaponTargetList) do
        ...
        local targetAllowed, targetPriority =
            g:AllowWeaponTarget(attackerID, targetID, attackerWeaponNum,
                                attackerWeaponDefID, priority)
...
function gadgetHandler:AllowWeaponInterceptTarget(interceptorUnitID,
                                                  interceptorWeaponNum,
                                                  targetProjectileID)
    for _, g in ipairs(self.AllowWeaponInterceptTargetList) do
        if (not g:AllowWeaponInterceptTarget(interceptorUnitID,
                                             interceptorWeaponNum,
                                             targetProjectileID)) then
            return false
        end
    end
...
function gadgetHandler:UnitFromFactory(unitID, unitDefID, unitTeam, factID,
                                       factDefID, userOrders)
    for _, g in ipairs(self.UnitFromFactoryList) do
        g:UnitFromFactory(unitID, unitDefID, unitTeam, factID, factDefID,
                          userOrders)
    end
    return
end

function gadgetHandler:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID,
                                     attackerDefID, attackerTeam, pre)
    if pre == false then return end
    for _, g in ipairs(self.UnitDestroyedList) do
        g:UnitDestroyed(unitID, unitDefID, unitTeam, attackerID, attackerDefID,
                        attackerTeam)
    end
    return
end
...
function gadgetHandler:UnitExperience(unitID, unitDefID, unitTeam, experience,
                                      oldExperience)
...
function gadgetHandler:UnitCmdDone(unitID, unitDefID, unitTeam, cmdID,
                                   cmdParams, cmdOptions, cmdTag)
    for _, g in ipairs(self.UnitCmdDoneList) do
        g:UnitCmdDone(unitID, unitDefID, unitTeam, cmdID, cmdParams, cmdOptions,
                      cmdTag)
    end
    return
end
...
function gadgetHandler:UnitPreDamaged(unitID, unitDefID, unitTeam, damage,
    ...
    for _, g in ipairs(self.UnitPreDamagedList) do
        local dmg, imp = g:UnitPreDamaged(unitID, unitDefID, unitTeam,
                                          retDamage, paralyzer, weaponDefID,
                                          projectileID, attackerID,
...
function gadgetHandler:UnitDamaged(unitID, unitDefID, unitTeam, damage,
                                   paralyzer, weaponID, projectileID,
                                   attackerID, attackerDefID, attackerTeam)
...
function gadgetHandler:UnitSeismicPing(x, y, z, strength, allyTeam, unitID,
                                       unitDefID)
...
function gadgetHandler:UnitLoaded(unitID, unitDefID, unitTeam, transportID,
                                  transportTeam)
...
function gadgetHandler:UnitUnloaded(unitID, unitDefID, unitTeam, transportID,
                                    transportTeam)
...
function gadgetHandler:StockpileChanged(unitID, unitDefID, unitTeam, weaponNum,
                                        oldCount, newCount)
    for _, g in ipairs(self.StockpileChangedList) do
        g:StockpileChanged(unitID, unitDefID, unitTeam, weaponNum, oldCount,
                           newCount)
    end
    return
end
...
function gadgetHandler:ShieldPreDamaged(proID, proOwnerID,
                                        shieldEmitterWeaponNum,
                                        shieldCarrierUnitID, bounceProjectile)

    for _, g in ipairs(self.ShieldPreDamagedList) do
        -- first gadget to handle this consumes the event
        if (g:ShieldPreDamaged(proID, proOwnerID, shieldEmitterWeaponNum,
                               shieldCarrierUnitID, bounceProjectile)) then
            return true
        end
    end
...
function gadgetHandler:Explosion(weaponID, px, py, pz, ownerID, proID)
    if Explosion_first then
        for _, g in ipairs(self.ExplosionList) do
            local weaponDefs = (g.Explosion_GetWantedWeaponDef and
                                   g:Explosion_GetWantedWeaponDef()) or
                                   allWeaponDefs
            for _, wdid in ipairs(weaponDefs) do
                if Explosion_GadgetSingle[wdid] or Explosion_GadgetMap[wdid] then
                    if Explosion_GadgetMap[wdid] then
                        Explosion_GadgetMap[wdid].count =
                            Explosion_GadgetMap[wdid].count + 1
                        Explosion_GadgetMap[wdid].data[Explosion_GadgetMap[wdid]
                            .count] = g
                    else
...
function gadgetHandler:Update()
    for _, g in ipairs(self.UpdateList) do g:Update() end
    return
end
...
function gadgetHandler:DrawGenesis()
    for _, g in ipairs(self.DrawGenesisList) do g:DrawGenesis() end
    return
end

function gadgetHandler:DrawWorld()
    for _, g in ipairs(self.DrawWorldList) do g:DrawWorld() end
    return
end

function gadgetHandler:DrawWorldPreUnit()
    for _, g in ipairs(self.DrawWorldPreUnitList) do g:DrawWorldPreUnit() end
    return
end

function gadgetHandler:DrawWorldShadow()
    for _, g in ipairs(self.DrawWorldShadowList) do g:DrawWorldShadow() end
    return
end
...
function gadgetHandler:DrawScreenPost(vsx, vsy)
    for _, g in ipairs(self.DrawScreenPostList) do g:DrawScreenPost(vsx, vsy) end
    return
end

function gadgetHandler:DrawScreen(vsx, vsy)
    for _, g in ipairs(self.DrawScreenList) do g:DrawScreen(vsx, vsy) end
    return
end

function gadgetHandler:DrawInMiniMap(mmsx, mmsy)
    for _, g in ipairs(self.DrawInMiniMapList) do g:DrawInMiniMap(mmsx, mmsy) end
    return
end
...
function gadgetHandler:KeyRelease(key, mods, label, unicode)
    for _, g in ipairs(self.KeyReleaseList) do
        if (g:KeyRelease(key, mods, label, unicode)) then return true end
    end
    return false
end
...
function gadgetHandler:MouseMove(x, y, dx, dy, button)
    local mo = self.mouseOwner
    if (mo and mo.MouseMove) then return mo:MouseMove(x, y, dx, dy, button) end
end

function gadgetHandler:MouseRelease(x, y, button)
    local mo = self.mouseOwner
    local mx, my, lmb, mmb, rmb = Spring.GetMouseState()
    if (not (lmb or mmb or rmb)) then self.mouseOwner = nil end
    if (mo and mo.MouseRelease) then return mo:MouseRelease(x, y, button) end
    return -1
end

function gadgetHandler:MouseWheel(up, value)
    for _, g in ipairs(self.MouseWheelList) do
        if (g:MouseWheel(up, value)) then return true end
    end
    return false
end
...
function gadgetHandler:IsAbove(x, y)
    for _, g in ipairs(self.IsAboveList) do
        if (g:IsAbove(x, y)) then return true end
    end
    return false
end
...
function gadgetHandler:GetTooltip(x, y)
    for _, g in ipairs(self.GetTooltipList) do
        if (g:IsAbove(x, y)) then
            ...
            if (string.len(tip) > 0) then return tip end
        end
    end
    return ''
end
...
function gadgetHandler:Save(zip)
    for _, g in ipairs(self.SaveList) do g:Save(zip) end
    return
end

function gadgetHandler:Load(zip)
    for _, g in ipairs(self.LoadList) do g:Load(zip) end
    return
end
...
function gadgetHandler:AllowCommand(unitID, unitDefID, unitTeam, cmdID,
                                    cmdParams, cmdOptions, cmdTag, synced) -- ours
    ...
    for _, g in ipairs(self.AllowCommandList) do
        if not AllowCommand_WantedCommand[g] then
            AllowCommand_WantedCommand[g] =
                (g.AllowCommand_GetWantedCommand and
                    g:AllowCommand_GetWantedCommand()) or true
        end
        if not AllowCommand_WantedUnitDefID[g] then
            AllowCommand_WantedUnitDefID[g] =
                (g.AllowCommand_GetWantedUnitDefID and
                    g:AllowCommand_GetWantedUnitDefID()) or true
        end
        ...
        if ((wantedCommand == true) or wantedCommand[cmdID]) and
            ...
            (not g:AllowCommand(unitID, unitDefID, unitTeam, cmdID, cmdParams,
                                cmdOptions, cmdTag, synced)) then -- ours
...
function gadgetHandler:RecvFromSynced(cmd, ...)
    ...
    if (actionHandler.RecvFromSynced(cmd, ...)) then return end
    for _, g in ipairs(self.RecvFromSyncedList) do
        if (g:RecvFromSynced(cmd, ...)) then return end
    end
    return
end
...
function gadgetHandler:GotChatMsg(msg, player)
    ...
    if (actionHandler.GotChatMsg(msg, player)) then return true end

    for _, g in ipairs(self.GotChatMsgList) do
        if (g:GotChatMsg(msg, player)) then return true end
    end
...
function gadgetHandler:UnitCommand(unitID, unitDefID, unitTeam, cmdID, cmdOpts,
                                   cmdParams) -- opts is a bitmask
...
function gadgetHandler:GameSetup(state, ready, playerStates)
    for _, g in ipairs(self.GameSetupList) do
        ...
        if (success) then return true, newReady end
    end
    return false
end

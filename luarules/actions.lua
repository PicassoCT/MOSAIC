-- $Id: actions.lua 3171 2008-11-06 09:06:29Z det $
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
--
--  file:    actions
...
--------------------------------------------------------------------------------
--------------------------------------------------------------------------------
if (actionHandler) then return actionHandler end
...
local function InsertCallInfo(callInfoList, gadget, func, help)
    ...
    for i, ci in ipairs(callInfoList) do
        ...
        if (layer >= g.ghInfo.layer) then index = i + 1 end
    end
...
local function RemoveAction(map, gadget, cmd)
    local callInfoList = map[cmd]
    if (callInfoList == nil) then return false end
    local count = RemoveCallInfo(callInfoList, gadget)
    if (#callInfoList <= 0) then map[cmd] = nil end
    return (count > 0)
end
...
local function Help(playerID, cmd)
    if (cmd == nil) then
        ...
        local sorted = {}
        for name in pairs(chatActions) do table.insert(sorted, name) end
        table.sort(sorted)
        local str = Script.GetName() .. ' commands: '
        for _, name in ipairs(sorted) do str = str .. '  ' .. name end
        Spring.Echo(str)
        if (isSyncedCode) then SendToUnsynced(playerID, "help") end
    else
        ...
        if (isSyncedCode) then SendToUnsynced(playerID, "help " .. cmd) end
    end
end
...
local function GotChatMsg(msg, playerID)
    ...
    for i, callInfo in ipairs(callInfoList) do
        ...
        if (func(cmd, msg, words, playerID)) then return true end
    end
...
local function RecvFromSynced(arg1, arg2, ...)
    if (type(arg1) == 'string') then
        ...
        for i, callInfo in ipairs(callInfoList) do
            local func = callInfo[1]
            if not func then
                Spring.Echo("actions.lua::no function with arguments" .. arg1 ..
                                " " .. arg2)

                return false
            else
                -- local gadget = callInfo[2]
                if (func(arg1, arg2, ...)) then return true end
            end
        end
        return false
    end
    ...
    if (type(arg1) == 'number') then
        -- a proxied chat msg
        if (type(arg2) == 'string') then return GotChatMsg(arg2, arg1) end
        return false
    end

...
vec4 terrainSurfaceWater(vec3 p, vec3 shadingNormal) {
    ...
    float banks=smoothstep(0.035,0.22,slope);
    float pixel=max(length(dFdx(p)),length(dFdy(p)));
    ...
    // Flat terrain keeps a uniform faint wet film and ripple shading, not a
    // flattened copy of the channel network. Sea-level rejection applies to all.
    vec4 film=vec4(0.12*terrainWetness,0.0,0.0,0.0);
    return mix(film,channels,banks)*smoothstep(0.0,1.5,p.y);
}
